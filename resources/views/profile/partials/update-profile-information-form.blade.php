<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <x-input-label for="location" :value="__('Ubicación por defecto')" />
            <select id="location" name="location" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
                <option value="">{{ __('Sin ubicación predeterminada') }}</option>
                @foreach(config('locations.cadiz') as $loc)
                    <option value="{{ $loc }}" {{ old('location', $user->location) === $loc ? 'selected' : '' }}>{{ $loc }}</option>
                @endforeach
            </select>
